...
const storeData = async (value) => {
  ...
  let result = await AsyncStorage.getItem('myrecipes');
  ...
  if (result != null) {
    ...
    if (!find) {
      ...
      await AsyncStorage.setItem('myrecipes', jsonValue);
    }
  } else {
  ...
  console.log(await AsyncStorage.getItem('myrecipes'));
};

function renderIngredients(element) {
  var num = 1;
  var elements = [];
  while (element.Ingredients[num] != undefined) {
    elements.push(
      <Text>
        {num} - {element.Ingredients[num]}
      </Text>
    );
    num++;
  }
